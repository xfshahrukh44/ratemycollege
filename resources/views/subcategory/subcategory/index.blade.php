    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="card">
            <div class="card-header bg-info">Subcategory</div>
            <div class="card-body">
                <a href="{{ url('/admin/subcategory/create') }}" class="btn btn-success btn-sm" title="Add New Subcategory">
                    <i class="fa fa-plus" aria-hidden="true"></i> Add New
                </a>

                <form method="GET" action="{{ url('/admin/subcategory') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                        ...
                        <tbody>
                            @foreach($subcategory as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('/admin/subcategory/' . $item->id) }}" title="View Subcategory"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View </button></a>
                                        <a href="{{ url('/admin/subcategory/' . $item->id . '/edit') }}" title="Edit Subcategory"><button class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i> Edit </button></a>

                                        <form method="POST" action="{{ url('/admin/subcategory' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                                            <button type="submit" class="btn btn-danger btn-sm" title="Delete Subcategory" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash" aria-hidden="true"></i> Delete </button>
                                        </form>
