                                ...
                                <div class="form-group">
                                    ...
                                    <br>
                                     ...
                                     <select id="single" class="form-control">
                                         <option value="{{ $get_coach_by_id->school_id }}"> {{ App\Models\Wp_school::find($get_coach_by_id->school_id)->name }} </option>
                                     </select>
                                    ...
                                    <br><br>
                                     ...
                                     <select  id="single3"  class="form-control">
                                         <option value="{{ $get_coach_by_id->sports_id }}"> {{ App\Models\Wp_sport::find($get_coach_by_id->sports_id)->name }} </option>
                                     </select>
                                     
                                    <br><br>
                                    
                                    <!--<label> Coach Name </label>-->
                                    <!-- <select  id="single4"  class="form-control">-->
                                        ...
                                        <div class="coach-profile-head d-flex border-bottom">
                                          ...
                                          <div class="coaches-profile-right-in-box-title">
                                            <h2>
                                              ...
                                              {{ $get_coach_by_id->name }} - {{ App\Models\Wp_sport::find($get_coach_by_id->sports_id)->name }} Coach
                                              
                                              <div class="school-name">
                                                <span style="margin-right: 10px;">
                                                  <img src="https://redcraftmedia.net/ratemycollege/wp-content/uploads/2024/02/Buildings.svg" >
                                                </span>
                                                <span>  {{ App\Models\Wp_school::find($get_coach_by_id->school_id)->name }} </span>
                                              </div>
                                            </h2>
                                          </div>
                                        </div>
                                    ...
                                    <?php $get_coaches = DB::table('wp_coaches')->take(20)->where('status', '1')->get(); ?>
                                    
                                    <form method="post" action="{{ route('add_change_coaches') }}">
                                        
                                      @csrf
                                      
                                      <input type="hidden" name="old_coach_id" value="{{ $get_coach_by_id->id }}" />  
                                      <input type="hidden" name="school_id" value="{{ $get_coach_by_id->school_id }}" />
                                      <input type="hidden" name="sports_id" value="{{ $get_coach_by_id->sports_id }}" />
                                        
                              
                                     <label> Add New Coach </label>
                                     <select  id="single4" class="form-control" name="new_coach_id">
                                         @foreach($get_coaches as $key => $val_coach)
